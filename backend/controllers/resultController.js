...
async function createResult(req, res) {
  try {
    const body = {...req.body};
    if(!body.studentId || !body.examName || !body.subject) return res.status(400).json({message:"Student, exam and subject are required"});
    ...
    if(Number(body.totalMarks)<=0 || Number(body.marksObtained)<0 || Number(body.marksObtained)>Number(body.totalMarks)) return res.status(400).json({message:"Marks must be between 0 and total marks"});
