...
export default function AdminFees() {
  ...
  const [tab, setTab] = useState(searchParams.get("tab") === "unpaid" ? "unpaid" : "records");
  return (
...
function UnpaidReport() {
  ...
  function load() {
    ...
    api.get("/fees/unpaid", { params }).then((res) => setRecords(res.data));
  }
  useEffect(load, [className, section]);

  function handleClassChange(value) {
    setClassName(value);
    setSection(""); // sections depend on the selected class, so reset it
  }
  ...
  const sectionOptions = classes.find((c) => c.name === className)?.Sections || [];

  return (
    <div className="space-y-6">
      <Card title="Unpaid Fees — by Class & Section">
        <div className="flex gap-3 mb-4">
          <Select value={className} onChange={(e) => handleClassChange(e.target.value)} className="w-48">
            <option value="">All Classes</option>
            {classes.map((c) => <option key={c.id} value={c.name}>{c.name}</option>)}
          </Select>
          <Select value={section} onChange={(e) => setSection(e.target.value)} className="w-40" disabled={!className}>
        ...
        <Table
          columns={[
            ...
            { key: "className", label: "Class", render: (r) => r.Student?.className },
            { key: "section", label: "Section", render: (r) => r.Student?.section || "—" },
            { key: "month", label: "Month" }, { key: "amountDue", label: "Due" }, { key: "amountPaid", label: "Paid" },
            { key: "status", label: "Status", render: (r) => <Badge status={r.status} /> },
      ...
      {records.length > 0 && (
        <Card title={className ? `Send Message — ${className}${section ? " " + section : ""}` : "Send Message — All Unpaid Guardians"}>
          ...
          <Button onClick={handleSendToAllUnpaid} disabled={!messageText}>Send Message to Unpaid Guardians</Button>
