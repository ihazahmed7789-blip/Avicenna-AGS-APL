...
export default function AdminClasses() {
  ...
  const [sectionForms, setSectionForms] = useState({}); // classId -> { name, group }
  ...
  async function handleAddSection(classId) {
    const data = sectionForms[classId] || {};
    if (!data.name) return;
    await api.post(`/classes/${classId}/sections`, { name: data.name, group: data.group || null });
    setSectionForms({ ...sectionForms, [classId]: { name: "", group: "" } });
    load();
  }
            ...
            <div key={cls.id} className="border border-[var(--color-line)] rounded-lg p-4">
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {cls.Sections?.map((s) => (
                  <span key={s.id} className="inline-flex items-center gap-1 bg-[var(--color-brand-light)] text-[var(--color-brand-dark)] text-xs px-2 py-1 rounded-full">
                    Section {s.name}{s.group && ` (${s.group})`}
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  value={sectionForms[cls.id]?.name || ""}
                  onChange={(e) => setSectionForms({ ...sectionForms, [cls.id]: { ...sectionForms[cls.id], name: e.target.value } })}
                  className="max-w-[140px]"
                />
                <Input
                  placeholder="Group (optional, e.g. Science)"
                  value={sectionForms[cls.id]?.group || ""}
                  onChange={(e) => setSectionForms({ ...sectionForms, [cls.id]: { ...sectionForms[cls.id], group: e.target.value } })}
