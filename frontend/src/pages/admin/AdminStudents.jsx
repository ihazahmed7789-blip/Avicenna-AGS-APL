...
const emptyForm = {
  rollNumber: "", fullName: "", fatherName: "", dateOfBirth: "", className: "", section: "",
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-3">
              ...
              <Field label="Class">
                <Select required value={form.className} onChange={(e) => setForm({ ...form, className: e.target.value, section: "" })}>
              ...
              <Field label="Section">
                <Select value={form.section || ""} onChange={(e) => setForm({ ...form, section: e.target.value })}>
                  ...
                  {classes.find((c) => c.name === form.className)?.Sections?.map((s) => (
                    <option key={s.id} value={s.name}>{s.name}{s.group ? ` (${s.group})` : ""}</option>
                  ))}
                </Select>
              </Field>
